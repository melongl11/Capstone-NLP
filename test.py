...
model1 = gensim.models.Word2Vec.load('weight/namuwiki-2-window10.model')
...
def get_similarities(sub_words):
    subw_len = len(sub_words)
    similarities = np.zeros((subw_len, subw_len))
    for i in range(subw_len):
        for j in range(subw_len):
            similarities[i][j] = get_similarity(sub_words[i], sub_words[j])
    return similarities


def get_correlation_score(word, word_list):
    return np.array([get_similarity(word, m) for m in word_list]).mean()

def get_block_score(word, center, related_words, lower_similarity_bound):
    block_score = get_similarity(word, center)
    block_score += np.array([lower_similarity_bound] + [get_similarity(word, m) for m in related_words if
                                                        get_similarity(word, m) > lower_similarity_bound]).mean()
...
##Local input init
def make_word_block(sub_words, sub_correlations, min_block_words_num, lower_similarity_bound, low_rate):
    ##Init
    word_block = {}

    # 유사도를 배열에 저장
    similarities = get_similarities(sub_words)
    ...
    while len(remain_words_idx) > 0 and len(must_have_words_idx) + 1 < min_block_words_num:
        ...
        score = get_block_score(sub_words[candidate_idx], sub_words[center_word_idx], related_words, lower_similarity_bound)
...
def clustering(input_sentence):
    ...
    for input_word in input_word_list:
        res = model1.wv.most_similar(input_word, topn=each_num)
        for m in res:
    ...
    for i in range(len(extracted_words)):
        ...
        related_words.append([word, get_correlation_score(word, input_word_list), extracted_correlation[i]])
    ...
    for i in range(100):  # max_blocks_num
        if len(local_input) > 2:
            (new_block, local_input) = make_word_block(local_input, local_correlation, min_block_words_num, lower_similarity_bound, low_rate)
    ...
    for i in range(len(local_input)):
        ...
        block_score = np.array([get_block_score(remained_word, m['center_word'], m['related_words'], lower_similarity_bound) for m in word_blocks])
    ...
    for block in result['blocks']:
        ...
        correlation = get_block_score(block['center_word'], input_word_list[0], input_word_list, lower_similarity_bound)
...
result = clustering("자동차")
d = dict()
d['n_cluster'] = result[0]
d['clusters'] = result[1]
